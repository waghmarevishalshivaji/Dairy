...
async function createVLCEntry(req, res) {
  const { date, shift, vlc_id, vlc_name, weight, fat, snf, clr, rate, amount } = req.body;
  if (!date || !shift || !vlc_id || !vlc_name || !weight || !fat || !snf || !clr || !rate || !amount) {
    return res.status(400).json({ success: false, message: 'All fields are required' });
  }
  try {
    const [result] = await db.execute(
      'INSERT INTO vlc_collection_entry (date, shift, vlc_id, vlc_name, weight, fat, snf, clr, rate, amount) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [date, shift, vlc_id, vlc_name, weight, fat, snf, clr, rate, amount]
    );
